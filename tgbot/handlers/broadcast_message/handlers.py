...
def broadcast_decision_handler(update: Update, context: CallbackContext) -> None:
    ...
    if broadcast_decision == CONFIRM_BROADCAST:
        ...
        user_ids = list(User.objects.prefetch_related('winner_user_challenges','user_challenge_answers','owner_user_challenges','opponent_user_challenges','user_user_challenges').all().values_list('user_id', flat=True))
