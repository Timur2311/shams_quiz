...
def random_opponent(update: Update, context: CallbackContext):
    ...
    user_random_challenges = UserChallenge.objects.prefetch_related('questions').prefetch_related('users').select_related('user').select_related(
        'opponent').select_related('challenge').filter(user__user_id=from_user_id, challenge__stage=created_user_challenge.challenge.stage, is_active=True, opponent=None)
    ...
    if possible_challenges.count() == 0:
        ...
        if user_random_challenges.count() == 0:
            my_message = update.callback_query.edit_message_text(f"Hozircha siz uchun raqib yo'q. Raqib topilishi bilan sizga xabar jo'natamiz. \n\n⚠️Raqib topilishini kutishni xohlamasangiz \"{consts.REVOKE}\" tugmasini bosing, aks holda \"Bosh Sahifa\" tugmasi orqali botdan foydalanishda davom etishingiz mumkin.", reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Bosh Sahifa", callback_data=f"home-page-{from_user_id}")], [InlineKeyboardButton(consts.REVOKE, callback_data=f"revoke-challenge-{from_user_id}-{created_user_challenge.id}")]]))
        elif user_random_challenges.count() == 1:
            my_message = update.callback_query.edit_message_text(f"{created_user_challenge.challenge.stage} - bosqich savollari bo'yicha sizga raqib qidirilmoqda, biror raqib ushbu bosqich savollari bo'yicha bellashishni istashi bilan sizga xabar beramiz. ⚠️Agar kutishni xohlamasangiz \"{consts.REVOKE}\" tugmasi orqali bellashuvni bekor qilishingiz mumkin,aks holda \"Bosh Sahifa\" tugmasi orqali botdan foydalanishda davom etishingiz mumkin.", reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Bosh Sahifa", callback_data=f"home-page-{from_user_id}")], [InlineKeyboardButton(consts.REVOKE, callback_data=f"revoke-challenge-{from_user_id}-{user_random_challenges.first().id}")]]))
        ...
        if user.user_id == user_challenge.user.user_id:  # user ekan
            ...
            if user_challenge.is_opponent_finished:
                ...
                not_finished_text = context.bot.send_message(
                    user.user_id, text=text, reply_markup = ReplyKeyboardMarkup([[consts.HOME_PAGE]]) , parse_mode=ParseMode.HTML)
